...
async function signConnect(dev, {token,role,scopes,signedAtMs,nonce}) {
  const v=nonce?'v2':'v1';
  const parts=[v,dev.id,'webchat','operator',role,scopes.join(','),String(signedAtMs),token||''];
...
async function wsConnect(url,token,browserId) {
  ...
  S.ws.onopen = async () => {
    ...
    const params={
      ...
      client:{id:'webchat',version:'1.71.3',platform:'browser_extension',mode:'operator'},
